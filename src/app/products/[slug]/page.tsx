...
import { getProductBySlug, getServicesByProduct, getCategoryForProduct, getProductsByCategory } from '@/lib/data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const category = getCategoryForProduct(product.id);
  return {
...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const category = getCategoryForProduct(product.id);
  const productServices = getServicesByProduct(product.id);
  const otherProducts = category
    ? getProductsByCategory(category.id).filter((p) => p.slug !== slug)
    : [];
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <div className="flex items-center gap-4 text-sm text-slate-500">
                <span>{productServices.length} services available</span>
                {productServices.length > 0 && (
                  <>
                    <span>&bull;</span>
                    <span>Starting from {formatPrice(Math.min(...productServices.map((s) => s.discountedPrice || s.price)))}</span>
                  </>
                )}
              </div>
            </div>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl font-semibold text-slate-900 mb-8">Available Services</h2>
          ...
          {productServices.length > 0 ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-500 mb-6">We&apos;re currently updating our services for this product.</p>
              {category && (
