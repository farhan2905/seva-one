...
import { getCategoryBySlug, getProductsByCategory, getServicesByProduct, getCategories } from '@/lib/data';
...
export default async function CategoryDetailPage({ params }: Props) {
  ...
  const categoryProducts = getProductsByCategory(category.id).map((p) => ({
    ...p,
    serviceCount: getServicesByProduct(p.id).length,
  }));
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative bg-gradient-to-br from-primary/5 via-cyan-500/5 to-indigo-500/5 py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">{category.name}</h1>
        ...
        <div className="container mx-auto px-4">
          ...
          {categoryProducts.length > 0 ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-500 mb-6">We&apos;re currently updating our services in this category.</p>
              <Button asChild variant="outline" className="gap-2">
                <Link href="/categories">Browse Other Services <ArrowRight className="w-4 h-4" /></Link>
              </Button>
            </div>
          )}
        </div>
      ...
      <section className="py-12 bg-slate-50">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl font-semibold text-slate-900 mb-6">Explore Other Services</h2>
